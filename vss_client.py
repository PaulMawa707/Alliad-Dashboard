...
BASE_URL: str = _env("VSS_BASE_URL")
USERNAME: str = _env("VSS_USERNAME")
PASSWORD_PLAINTEXT: str = _env("VSS_PASSWORD")
...
def stored_vss_token_expired() -> bool:
    """True when the in-memory/Neon token is older than ``VSS_TOKEN_TTL_HOURS``."""
    return _memory_token_expired()


def _retry_on_session_expired(call_fn):
    """Run ``call_fn(token)``; on 10023 reload stored token, then apiLogin from .env.

    Auto-refresh / cron set ``vss_no_login_mode`` so a dead session never calls
    apiLogin (that path is rate-limited with 10082). The 12h token job refreshes Neon.
    """
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pass
            if _vss_no_login() or not _vss_credentials_in_env() or _login_cooldown_active():
                err = (
...
def discover_fleets_by_name(*, contains: str) -> list[tuple[str, str]]:
    """Return [(fleet_id, fleet_name)] for any fleet name containing ``contains``."""
    return discover_dhl_fleets(contains=contains)


def discover_fleet_devices_by_name(
    *,
    contains: str,
    page_size: int = 200,
    max_pages: int = 60,
    ...
    """Devices under fleets whose name contains ``contains`` (ignores DHL_* env vars)."""
