"""Background VSS/MiX cache prewarm (ported from app.py)."""

from __future__ import annotations

import logging
import os
import threading
import time
from contextlib import nullcontext

import operation_log

from data import (
    cache_age_seconds,
    cache_get,
    load_alarms_last_24h,
    load_dhl_devices,
    load_mix_health,
    mix_integration_enabled,
    refresh_alarms_last_24h,
    refresh_behaviour_events,
    refresh_dhl_devices,
    refresh_mix_health,
    refresh_realtime_status,
    refresh_track3_units,
    load_realtime_status,
    seed_device_cache_from_snapshot,
    track3_integration_enabled,
    _realtime_live_age_count,
)
from vss_client import (
    active_base_url,
    ensure_token,
    get_current_token,
    keepalive_ping,
    last_vss_error,
    last_vss_profile,
    last_vss_token_source,
    refresh_token_if_expired,
    stored_vss_token_expired,
    try_token_without_login,
    validate_or_renew_token,
    vss_no_login_mode,
)

log = logging.getLogger("dhl-flask")

try:
    REFRESH_MINUTES = max(1, int(os.environ.get("DHL_AUTO_REFRESH_MINUTES", "60").strip() or "60"))
except ValueError:
    REFRESH_MINUTES = 60

KEEPALIVE_MINUTES = 20
TOKEN_CHECK_MINUTES = 30


def realtime_auto_refresh_seconds() -> int:
    """How stale realtime status may get before it is re-pulled; 0 disables it."""
    try:
        minutes = int(os.environ.get("DHL_REALTIME_AUTO_REFRESH_MINUTES", "30").strip() or "30")
    except ValueError:
        minutes = 30
    if minutes <= 0:
        return 0
    return max(60, minutes * 60)


def realtime_data_age_seconds() -> float | None:
    """Age of the realtime snapshot in seconds (None when nothing is loaded).

    Snapshots hydrated from Neon carry their original ``updated_at``, so this
    stays accurate across serverless cold starts.
    """
    return cache_age_seconds("realtime_status")

_prewarm_mix_lock = threading.Lock()
_prewarm_mix_running = False
_prewarm_mix_pending = False
_prewarm_vss_lock = threading.Lock()
_prewarm_vss_running = False
_prewarm_vss_pending = False


def _prewarm_mix(*, stale_while_revalidate: bool) -> None:
    if not mix_integration_enabled():
        return
    load_fn = refresh_mix_health if stale_while_revalidate else load_mix_health
    label = "background refresh" if stale_while_revalidate else "startup"
    try:
        log.info("prewarm: mix_health starting (%s)", label)
        operation_log.log_event("mix_data", "fetch_mix_health", "running", f"MiX health fetch starting ({label})")
        load_fn()
        log.info("prewarm: mix_health done")
        operation_log.log_event("mix_data", "fetch_mix_health", "ok", f"MiX health fetch complete ({label})")
    except Exception as e:  # noqa: BLE001
        log.warning("prewarm: mix_health failed: %s", e)
        operation_log.log_event("mix_data", "fetch_mix_health", "error", f"MiX health fetch failed: {e}")


def _prewarm_vss(
    *,
    keep_devices: bool,
    stale_while_revalidate: bool,
    allow_token_reauth: bool = False,
) -> None:
    token: str | None = None
    pid: str = ""

    if stale_while_revalidate:
        cached = get_current_token() or try_token_without_login()
        if cached:
            token, pid = cached[0], cached[1] or ""
            log.info("prewarm: VSS token %s... (cached)", token[:12])
        else:
            log.info("prewarm: refresh — no cached VSS token; acquiring via Neon/env/login")
            try:
                token, pid = ensure_token(login_max_wait_seconds=120, allow_10082_retry=True)
                log.info(
                    "prewarm: VSS token %s... (%s) profile=%s base=%s",
                    token[:12],
                    last_vss_token_source() or "unknown",
                    last_vss_profile() or "?",
                    active_base_url(),
                )
            except Exception as e:  # noqa: BLE001
                err = last_vss_error() or str(e)
                log.warning("prewarm: VSS token unavailable: %s", err)
                return
    else:
        try:
            token, pid = ensure_token(login_max_wait_seconds=120, allow_10082_retry=True)
            log.info(
                "prewarm: VSS token %s... (%s) profile=%s base=%s",
                token[:12],
                last_vss_token_source() or "unknown",
                last_vss_profile() or "?",
                active_base_url(),
            )
        except Exception as e:  # noqa: BLE001
            err = last_vss_error() or str(e)
            log.warning("prewarm: VSS token unavailable: %s", err)
            return

    token_ctx = vss_no_login_mode() if stale_while_revalidate else nullcontext()
    with token_ctx:
        ok, msg = validate_or_renew_token(
            allow_reauth=allow_token_reauth or not stale_while_revalidate
        )
        if not ok:
            log.warning("prewarm: VSS token check: %s", msg)
            return
        else:
            log.info("prewarm: VSS token validated")

        load_devices = refresh_dhl_devices if stale_while_revalidate else load_dhl_devices
        load_realtime = refresh_realtime_status if stale_while_revalidate else load_realtime_status
        load_alarms = refresh_alarms_last_24h if stale_while_revalidate else load_alarms_last_24h

        need_devices = cache_get("dhl_devices") is None or (stale_while_revalidate and not keep_devices)
        if need_devices:
            try:
                log.info("prewarm: dhl_devices starting")
                operation_log.log_event("vss_data", "fetch_dhl_devices", "running", "VSS device list fetch starting")
                load_devices()
                log.info("prewarm: dhl_devices done")
                operation_log.log_event("vss_data", "fetch_dhl_devices", "ok", "VSS device list fetch complete")
            except Exception as e:  # noqa: BLE001
                log.warning("prewarm: dhl_devices failed: %s", e)
                operation_log.log_event("vss_data", "fetch_dhl_devices", "error", f"VSS device list failed: {e}")
        else:
            log.info("prewarm: dhl_devices already cached")

        for name, fn, cache_key in (
            ("realtime_status", load_realtime, "fetch_realtime"),
            ("alarms_last_24h", load_alarms, "fetch_alarms"),
        ):
            try:
                log.info("prewarm: %s starting", name)
                operation_log.log_event("vss_data", cache_key, "running", f"VSS {name} fetch starting")
                fn()
                log.info("prewarm: %s done", name)
                operation_log.log_event("vss_data", cache_key, "ok", f"VSS {name} fetch complete")
            except Exception as e:  # noqa: BLE001
                log.warning("prewarm: %s failed: %s", name, e)
                operation_log.log_event("vss_data", cache_key, "error", f"VSS {name} failed: {e}")


def _kick_mix_prewarm(*, stale_while_revalidate: bool = False) -> None:
    global _prewarm_mix_running, _prewarm_mix_pending

    with _prewarm_mix_lock:
        if _prewarm_mix_running:
            _prewarm_mix_pending = True
            return
        _prewarm_mix_running = True

    def _runner() -> None:
        global _prewarm_mix_running, _prewarm_mix_pending
        pending = False
        swr = stale_while_revalidate
        try:
            _prewarm_mix(stale_while_revalidate=swr)
        finally:
            with _prewarm_mix_lock:
                _prewarm_mix_running = False
                pending = _prewarm_mix_pending
                _prewarm_mix_pending = False
            if pending:
                _kick_mix_prewarm(stale_while_revalidate=swr)

    threading.Thread(target=_runner, daemon=True, name="dhl-prewarm-mix").start()


def _kick_vss_prewarm(*, keep_devices: bool = False, stale_while_revalidate: bool = False) -> None:
    global _prewarm_vss_running, _prewarm_vss_pending

    with _prewarm_vss_lock:
        if _prewarm_vss_running:
            _prewarm_vss_pending = True
            return
        _prewarm_vss_running = True

    def _runner() -> None:
        global _prewarm_vss_running, _prewarm_vss_pending
        pending = False
        kd = keep_devices
        swr = stale_while_revalidate
        try:
            _prewarm_vss(keep_devices=kd, stale_while_revalidate=swr)
        finally:
            with _prewarm_vss_lock:
                _prewarm_vss_running = False
                pending = _prewarm_vss_pending
                _prewarm_vss_pending = False
            if pending:
                _kick_vss_prewarm(keep_devices=kd, stale_while_revalidate=swr)

    threading.Thread(target=_runner, daemon=True, name="dhl-prewarm-vss").start()


_prewarm_track3_lock = threading.Lock()
_prewarm_track3_running = False


def _kick_track3_prewarm(*, stale_while_revalidate: bool) -> None:
    global _prewarm_track3_running
    if not track3_integration_enabled() and not mix_integration_enabled():
        return
    with _prewarm_track3_lock:
        if _prewarm_track3_running:
            return
        _prewarm_track3_running = True

    def _runner() -> None:
        global _prewarm_track3_running
        try:
            _prewarm_track3(stale_while_revalidate=stale_while_revalidate)
        finally:
            with _prewarm_track3_lock:
                _prewarm_track3_running = False

    threading.Thread(target=_runner, daemon=True, name="alliad-prewarm-track3").start()


def _prewarm_track3(*, stale_while_revalidate: bool) -> None:
    """Track3 units + driver behaviour. Never raises: the page shows the banner."""
    if not track3_integration_enabled() and not mix_integration_enabled():
        return
    label = "background refresh" if stale_while_revalidate else "startup"
    if track3_integration_enabled():
        try:
            log.info("prewarm: track3_units starting (%s)", label)
            refresh_track3_units()
            log.info("prewarm: track3_units done")
        except Exception as e:  # noqa: BLE001
            log.warning("prewarm: track3_units failed: %s", e)
            operation_log.log_event(
                "track3_data", "fetch_track3_units", "error", f"Track3 unit list failed: {e}"
            )
    try:
        log.info("prewarm: behaviour_events starting (%s)", label)
        refresh_behaviour_events()
        log.info("prewarm: behaviour_events done")
    except Exception as e:  # noqa: BLE001
        log.warning("prewarm: behaviour_events failed: %s", e)
        operation_log.log_event(
            "track3_data", "fetch_behaviour", "error", f"Driver-behaviour events failed: {e}"
        )


def prewarm_cache(*, keep_devices: bool = False, stale_while_revalidate: bool = False) -> None:
    _kick_mix_prewarm(stale_while_revalidate=stale_while_revalidate)
    _kick_track3_prewarm(stale_while_revalidate=stale_while_revalidate)
    _kick_vss_prewarm(keep_devices=keep_devices, stale_while_revalidate=stale_while_revalidate)


def prewarm_cache_sync(*, keep_devices: bool = False, stale_while_revalidate: bool = False) -> None:
    """Run prewarm in the current thread (manual Refresh data only)."""
    _prewarm_track3(stale_while_revalidate=stale_while_revalidate)
    _prewarm_mix(stale_while_revalidate=stale_while_revalidate)
    _prewarm_vss(
        keep_devices=keep_devices,
        stale_while_revalidate=stale_while_revalidate,
        allow_token_reauth=True,
    )


def auto_refresh_track3(*, force: bool = False) -> dict[str, object]:
    """Scheduled Track3/behaviour pull, mirroring the realtime auto-refresh window."""
    interval = realtime_auto_refresh_seconds()
    if not interval and not force:
        return {"refreshed": False, "reason": "disabled"}

    out: dict[str, object] = {}
    for key, fn in (
        ("track3_units", refresh_track3_units),
        ("behaviour_events", refresh_behaviour_events),
    ):
        if key == "track3_units" and not track3_integration_enabled():
            continue
        age = cache_age_seconds(key)
        if not force and age is not None and interval and age < interval:
            out[key] = "fresh"
            continue
        try:
            df = fn()
            out[key] = int(len(df))
        except Exception as e:  # noqa: BLE001
            log.warning("auto-refresh: %s failed: %s", key, e)
            out[key] = f"error: {e}"
    return {"refreshed": any(isinstance(v, int) for v in out.values()), "track3": out}


# Manual refresh split into stages: a full pull of every platform exceeds the
# 300s serverless limit, so the browser calls one stage per request.
# Track3 and MiX run first — they are the quick stages, so those pages fill in
# without waiting on the multi-minute VSS crawl.
REFRESH_STEPS = (
    "track3_units",
    "behaviour",
    "mix",
    "vss_devices",
    "vss_realtime",
    "vss_alarms",
)


def _acquire_vss_token() -> str | None:
    """Return an error message when no live VSS token can be obtained."""
    try:
        token, _pid = ensure_token(login_max_wait_seconds=120, allow_10082_retry=True)
        log.info(
            "refresh: VSS token %s... (%s) profile=%s base=%s",
            token[:12],
            last_vss_token_source() or "unknown",
            last_vss_profile() or "?",
            active_base_url(),
        )
    except Exception as e:  # noqa: BLE001
        return last_vss_error() or str(e)

    ok, msg = validate_or_renew_token(allow_reauth=True)
    if not ok:
        return msg or last_vss_error() or "VSS token validation failed"
    return None


def _hydrate_devices_for_step() -> None:
    """Each stage runs in its own process — reload the device list from the database."""
    from data import hydrate_missing_snapshots_from_neon

    if cache_get("dhl_devices") is None:
        try:
            hydrate_missing_snapshots_from_neon(exclude_keys={"realtime_status", "alarms_24h", "mix_health"})
        except Exception as e:  # noqa: BLE001
            log.warning("refresh: device hydrate failed: %s", e)


def refresh_step(step: str) -> tuple[bool, str | None]:
    """Run a single manual-refresh stage. Returns ``(ok, error_message)``."""
    if step == "track3_units":
        if not track3_integration_enabled():
            return True, None
        try:
            operation_log.log_event(
                "track3_data", "fetch_track3_units", "running", "Track3 unit list starting (refresh)"
            )
            df = refresh_track3_units()
            operation_log.log_event(
                "track3_data", "fetch_track3_units", "ok", f"Track3 unit list complete ({len(df)} units)"
            )
            return True, None
        except Exception as e:  # noqa: BLE001
            msg = str(e)
            operation_log.log_event(
                "track3_data", "fetch_track3_units", "error", f"Track3 unit list failed: {msg}"
            )
            return False, msg

    if step == "behaviour":
        if not track3_integration_enabled() and not mix_integration_enabled():
            return True, None
        try:
            operation_log.log_event(
                "track3_data", "fetch_behaviour", "running", "Driver-behaviour events starting (refresh)"
            )
            df = refresh_behaviour_events()
            operation_log.log_event(
                "track3_data",
                "fetch_behaviour",
                "ok",
                f"Driver-behaviour events complete ({len(df)} rows)",
            )
            from data import last_behaviour_error

            partial = last_behaviour_error()
            return True, partial
        except Exception as e:  # noqa: BLE001
            msg = str(e)
            operation_log.log_event(
                "track3_data", "fetch_behaviour", "error", f"Driver-behaviour events failed: {msg}"
            )
            return False, msg

    if step == "mix":
        if not mix_integration_enabled():
            return True, None
        try:
            operation_log.log_event("mix_data", "fetch_mix_health", "running", "MiX health fetch starting (refresh)")
            refresh_mix_health()
            operation_log.log_event("mix_data", "fetch_mix_health", "ok", "MiX health fetch complete")
            return True, None
        except Exception as e:  # noqa: BLE001
            msg = str(e)
            operation_log.log_event("mix_data", "fetch_mix_health", "error", f"MiX health fetch failed: {msg}")
            return False, msg

    token_error = _acquire_vss_token()
    if token_error:
        operation_log.log_event("vss_data", "token", "error", f"VSS token unavailable: {token_error}")
        return False, token_error

    if step == "vss_devices":
        name, fn, log_step = "dhl_devices", refresh_dhl_devices, "fetch_dhl_devices"
    elif step == "vss_realtime":
        _hydrate_devices_for_step()
        name, fn, log_step = "realtime_status", refresh_realtime_status, "fetch_realtime"
    elif step == "vss_alarms":
        _hydrate_devices_for_step()
        name, fn, log_step = "alarms_last_24h", refresh_alarms_last_24h, "fetch_alarms"
    else:
        return False, f"Unknown refresh step: {step}"

    try:
        operation_log.log_event("vss_data", log_step, "running", f"VSS {name} fetch starting")
        fn()
        operation_log.log_event("vss_data", log_step, "ok", f"VSS {name} fetch complete")
        return True, None
    except Exception as e:  # noqa: BLE001
        msg = str(e)
        operation_log.log_event("vss_data", log_step, "error", f"VSS {name} failed: {msg}")
        return False, msg


_auto_realtime_lock = threading.Lock()


def auto_refresh_realtime(*, force: bool = False) -> dict[str, object]:
    """Re-pull realtime status when it is stale, reusing the stored VSS token.

    Normal ticks never call apiLogin. If VSS returns 10023 the stored *session*
    is dead (idle timeout) even when our 22h TTL has not elapsed — then we
    apiLogin once, with a Neon cooldown so overlapping crons cannot 10082.
    """
    interval = realtime_auto_refresh_seconds()
    if not interval and not force:
        return {"refreshed": False, "reason": "disabled"}

    age = realtime_data_age_seconds()
    if not force and age is not None and age < interval:
        return {"refreshed": False, "reason": "fresh", "age_seconds": int(age)}

    if not _auto_realtime_lock.acquire(blocking=False):
        return {"refreshed": False, "reason": "busy", "age_seconds": int(age) if age is not None else None}

    import neon_meta_store

    def _finish_ok(df) -> dict[str, object]:
        live = _realtime_live_age_count(df)
        rows = int(len(df)) if df is not None else 0
        if live == 0:
            operation_log.log_event(
                "vss_data",
                "fetch_realtime",
                "error",
                "Auto-refresh: VSS returned no last-seen times (online/offline unknown)",
            )
            return {"refreshed": False, "reason": "no-status-times", "rows": rows}
        operation_log.log_event(
            "vss_data",
            "fetch_realtime",
            "ok",
            f"Auto-refresh: realtime status complete ({rows} devices, {live} with last seen)",
        )
        log.info("auto-refresh: realtime status updated (%s devices, %s live ages)", rows, live)
        try:
            neon_meta_store.record_last_auto_refresh(rows=rows, source="auto")
        except Exception as meta_exc:  # noqa: BLE001
            log.debug("auto-refresh: could not store last time: %s", meta_exc)
        return {"refreshed": True, "rows": rows, "age_seconds": 0}

    try:
        session_dead = False
        with vss_no_login_mode():
            if try_token_without_login() and not stored_vss_token_expired():
                log.info(
                    "auto-refresh: using stored VSS token %s... (%s) — no apiLogin",
                    (get_current_token() or ("", ""))[0][:12],
                    last_vss_token_source() or "stored",
                )
                _hydrate_devices_for_step()
                operation_log.log_event(
                    "vss_data",
                    "fetch_realtime",
                    "running",
                    f"Auto-refresh: realtime status ({interval // 60} min, stored token)",
                )
                try:
                    return _finish_ok(refresh_realtime_status())
                except Exception as pull_exc:  # noqa: BLE001
                    msg = str(pull_exc)
                    if "10023" not in msg.lower() and "session has expired" not in msg.lower():
                        raise
                    session_dead = True
                    log.info("auto-refresh: stored VSS session rejected (10023)")
            else:
                session_dead = True
                log.info("auto-refresh: no usable stored VSS token")

        if not session_dead:
            return {"refreshed": False, "reason": "error"}

        last_login = neon_meta_store.seconds_since_vss_login_attempt()
        if last_login is not None and last_login < 600:
            return {
                "refreshed": False,
                "reason": "login-cooldown",
                "message": f"VSS apiLogin skipped — last attempt {int(last_login)}s ago (avoid 10082)",
            }
        neon_meta_store.record_vss_login_attempt(reason="10023")
        operation_log.log_event(
            "vss_data",
            "token",
            "running",
            "VSS apiLogin once — stored session expired (10023)",
        )
        try:
            ensure_token(
                force=True,
                skip_file=True,
                login_max_wait_seconds=45,
                allow_10082_retry=False,
            )
        except Exception as login_exc:  # noqa: BLE001
            log.warning("auto-refresh: VSS apiLogin failed: %s", login_exc)
            return {"refreshed": False, "reason": "login-failed", "message": str(login_exc)}
        _hydrate_devices_for_step()
        operation_log.log_event(
            "vss_data",
            "fetch_realtime",
            "running",
            f"Auto-refresh: realtime status ({interval // 60} min, after apiLogin)",
        )
        return _finish_ok(refresh_realtime_status())
    except Exception as exc:  # noqa: BLE001
        msg = str(exc)
        log.warning("auto-refresh: realtime status failed: %s", msg)
        operation_log.log_event("vss_data", "fetch_realtime", "error", f"Auto-refresh failed: {msg}")
        return {"refreshed": False, "reason": "error", "message": msg}
    finally:
        _auto_realtime_lock.release()


_local_cron_started = False


def _start_local_realtime_cron() -> None:
    """In-process 30-minute timer for local Flask. Vercel uses vercel.json crons."""
    global _local_cron_started
    if os.environ.get("VERCEL", "").strip():
        return
    interval = realtime_auto_refresh_seconds()
    if interval <= 0 or _local_cron_started:
        return
    _local_cron_started = True

    def _loop() -> None:
        log.info("local realtime cron: every %s minutes", max(1, interval // 60))
        while True:
            time.sleep(interval)
            try:
                result = auto_refresh_realtime(force=True)
                log.info(
                    "local realtime cron: refreshed=%s reason=%s",
                    result.get("refreshed"),
                    result.get("reason"),
                )
            except Exception as exc:  # noqa: BLE001
                log.warning("local realtime cron failed: %s", exc)

    threading.Thread(target=_loop, name="dhl-realtime-cron", daemon=True).start()


def start_background_workers() -> None:
    """Startup: hydrate display cache from Neon only — no VSS/MiX API calls."""
    try:
        from data import cache_needs_hydration, hydrate_cache_from_neon

        if cache_needs_hydration():
            hydrate_cache_from_neon()
            log.info("startup: dashboard cache hydrated from Neon")
    except Exception as e:  # noqa: BLE001
        log.warning("startup Neon hydrate skipped: %s", e)
    # Serverless page loads must not poke VSS — that sets a 10023 banner on cached data.
    # Local Flask still prewarms. Production realtime is the 15-minute cron.
    if os.environ.get("VERCEL", "").strip() != "1":
        prewarm_cache(stale_while_revalidate=True)
    _start_local_realtime_cron()
