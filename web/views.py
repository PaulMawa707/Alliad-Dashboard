...
from data import (
    ...
    get_alarms_cached,
    get_behaviour_events_cached,
    get_dhl_devices_cached,
    get_mix_health_cached,
    get_realtime_cached,
    get_track3_units_cached,
    last_behaviour_error,
    last_mix_error,
    last_saved_refresh_display,
    last_track3_error,
    load_alarms_last_24h,
    load_behaviour_events,
    load_dhl_devices,
    load_mix_health,
...
def nav_items(*, active: str, mix_enabled: bool, track3_enabled: bool = False) -> list[dict]:
    items = [
    ...
    if track3_enabled:
        items.insert(2, {"id": "track3", "label": "Track3 Fleet", "href": "/dashboard/track3", "icon": "truck"})
    if mix_enabled:
        items.insert(2 if not track3_enabled else 3, {"id": "mix", "label": "MiX Health", "href": "/dashboard/mix", "icon": "satellite"})
    for item in items:
...
def _device_picker_options(*, dhl_only: bool = True) -> list[dict]:
    """Device picker options — this customer's fleet assets from the dashboard cache."""
    devices = _devices_df()
...
def camera_context(
    *,
    ...
    return {
        "title": "Camera",
        "subtitle": (
            f"Search a {brand.brand_name()} fleet vehicle and view live VSS feeds on CH1–CH4 "
            "(same device list as Real-Time Status)."
        ),
...
def mix_context(*, issues: list[str] | None = None) -> dict[str, Any]:
    ...
    if df is None:
        return {
            "title": "MiX Telematics",
            "subtitle": f"{brand.brand_name()} site on MiX — health flags and diagnostics.",
            "enabled": True,
            "notice": mix_err or "",
            "loading": not mix_err,
...
# --------------------------------------------------------------------------- #
# Driver behaviour (Track3 eco-driving + MiX events)
# --------------------------------------------------------------------------- #

BEHAVIOUR_CHARTS = [
    {"value": "per_asset", "label": "Worst vehicles"},
    {"value": "mix", "label": "Violation mix"},
    {"value": "trend", "label": "Per-hour trend"},
    {"value": "source", "label": "By platform"},
    {"value": "map", "label": "Map"},
]

BEHAVIOUR_TABLE_COLUMNS = [
    "Start",
    "Asset",
    "Registration",
    "Behaviour",
...
def _behaviour_window_label() -> str:
    import track3_events

    hours = track3_events.behaviour_hours()
...
def _behaviour_df() -> pd.DataFrame | None:
    df = get_behaviour_events_cached()
    if df is not None or not _sync_load_on_page():
        return df
    try:
        return load_behaviour_events()
    except Exception:
        return get_behaviour_events_cached()
...
def behaviour_context(
    *,
    ...
    """Overspeeding and harsh driving across Track3 and MiX."""
    selected_behaviours = _parse_multi(behaviours)
    selected_sources = _parse_multi(sources)
    window_label = _behaviour_window_label()
    chart = chart if chart in {c["value"] for c in BEHAVIOUR_CHARTS} else "per_asset"

    base = {
        "title": "Driver Behaviour",
        "subtitle": (
            f"Overspeeding and harsh driving for the {brand.brand_name()} fleet "
            f"({window_label}) — Track3 eco-driving and MiX events."
        ),
        "behaviour_opts": list(BEHAVIOURS),
        "behaviours": selected_behaviours,
        "source_opts": [SOURCE_TRACK3, SOURCE_MIX],
        "sources": selected_sources,
        "chart_opts": BEHAVIOUR_CHARTS,
        "chart": chart,
        "window_label": window_label,
    }

    df = _behaviour_df()
    err = last_behaviour_error()
    if df is None:
        return {
            **base,
            "notice": err or "",
            "loading": not err,
            "kpis": [],
            "charts": [figure_html(C.loading_fig("Loading driver behaviour…"))] if not err else [],
            "table_html": df_to_table_html(None) if not err else "",
        }

    filtered = df.copy()
    if selected_behaviours:
        filtered = filtered[filtered["Behaviour"].astype(str).isin(selected_behaviours)]
    if selected_sources:
        filtered = filtered[filtered["Source"].astype(str).isin(selected_sources)]

    counts = behaviour_counts(filtered)
    per_asset = per_asset_counts(filtered)
    vehicles_flagged = int(len(per_asset))
    total = int(sum(counts.values()))

    kpis = [
        kpi_dict(
            "Over speeding",
            f"{counts[OVER_SPEEDING]:,}",
            accent=C.BEHAVIOUR_COLOR_OVERSPEED,
            border_accent=C.BEHAVIOUR_COLOR_OVERSPEED,
            sub=window_label,
        ),
        kpi_dict(
            "Harsh acceleration",
            f"{counts[HARSH_ACCELERATION]:,}",
            accent=C.BEHAVIOUR_COLOR_ACCEL,
            border_accent=C.BEHAVIOUR_COLOR_ACCEL,
            sub=window_label,
        ),
        kpi_dict(
            "Harsh braking",
            f"{counts[HARSH_BRAKING]:,}",
            accent=C.BEHAVIOUR_COLOR_BRAKE,
            border_accent=C.BEHAVIOUR_COLOR_BRAKE,
            sub=window_label,
        ),
        kpi_dict(
            "Harsh cornering",
            f"{counts[HARSH_CORNERING]:,}",
            accent=C.BEHAVIOUR_COLOR_CORNER,
            border_accent=C.BEHAVIOUR_COLOR_CORNER,
            sub=window_label,
        ),
        kpi_dict(
            "Vehicles flagged",
            ...
            border_accent=C.BRAND_PRIMARY,
            sub=f"{total:,} violations total",
        ),
    ]

    chart_builders = {
        "per_asset": lambda: C.behaviour_per_asset_bar(per_asset, window_label=window_label),
        "mix": lambda: C.behaviour_mix_donut(counts, window_label=window_label),
        "trend": lambda: C.behaviour_trend_line(filtered, window_label=window_label),
    ...
    if "Start" in table_df.columns:
        table_df["Start"] = pd.to_datetime(table_df["Start"], errors="coerce", utc=True).dt.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    show_cols = [c for c in BEHAVIOUR_TABLE_COLUMNS if c in table_df.columns]

    notice = err or ""
    if filtered.empty and not err:
        notice = f"No violations recorded in the {window_label} for the current filters."
    ...
    return {
        **base,
        ...
        "charts": [chart_html],
        "chart_html": chart_html,
        "table_html": df_to_table_html(table_df, show_cols, page_size=25),
        "per_asset_table_html": df_to_table_html(
            per_asset, ["Asset", *BEHAVIOURS, "Total"], page_size=10
        ),
    }
...
def _track3_df() -> pd.DataFrame | None:
    df = get_track3_units_cached()
    if df is not None or not _sync_load_on_page():
        return df
    try:
        return load_track3_units()
    except Exception:
        return get_track3_units_cached()


def track3_context(*, statuses: list[str] | None = None) -> dict[str, Any]:
    """Track3 (Wialon) unit inventory with reporting status."""
    import track3_client as t3

    selected = _parse_multi(statuses)
    online_limit = t3.online_minutes()
    base = {
        "title": "Track3 Fleet",
        "subtitle": (
            f"{t3.group_name()} unit group on Track3 — reporting status from the last "
            f"Wialon message (online = within {online_limit} min)."
        ),
        "status_opts": ["Online", "Offline", "Unknown"],
        "statuses": selected,
        "enabled": t3.track3_enabled(),
    }

    if not t3.track3_enabled():
        return {
            **base,
            "notice": "Track3 is not configured — set TRACK3_TOKEN and TRACK3_GROUP_ID.",
            "loading": False,
            "kpis": [],
            "charts": [],
            "table_html": "",
    ...
    if df is None:
        return {
            **base,
            ...
            "loading": not err,
            "kpis": [],
            "charts": [figure_html(C.loading_fig("Loading Track3 units…"))] if not err else [],
            "table_html": df_to_table_html(None) if not err else "",
        }

    filtered = df.copy()
    if selected and "Status" in filtered.columns:
    ...
    online = int(status_counts.get("Online", 0))
    offline = int(status_counts.get("Offline", 0))
    unknown = int(status_counts.get("Unknown", 0))
    moving = int(df["Moving"].fillna(False).astype(bool).sum()) if "Moving" in df.columns else 0

    kpis = [
        kpi_dict("Units", f"{len(df):,}", accent=C.BRAND_PRIMARY, border_accent=C.BRAND_PRIMARY),
        kpi_dict("Online", f"{online:,}", accent="#2E8B57", border_accent="#2E8B57", sub=f"message ≤ {online_limit} min"),
        kpi_dict("Offline", f"{offline:,}", accent=DHL_RED, border_accent=DHL_RED, sub=f"no message in {online_limit} min"),
        kpi_dict("Moving now", f"{moving:,}", accent=C.BRAND_SECONDARY, border_accent=C.BRAND_SECONDARY, sub="speed > 3 km/h"),
    ]
    if unknown:
        kpis.append(kpi_dict("No position", f"{unknown:,}", accent="#6B7280", border_accent="#6B7280"))
    ...
    return {
        **base,
        ...
        "kpis": kpis,
        "charts": charts,
        "table_html": df_to_table_html(filtered, show_cols, page_size=25),
    }
