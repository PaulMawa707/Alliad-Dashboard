...
LAST_VSS_LOGIN_KEY = "last_vss_login_attempt"
...
def configured() -> bool:
    return db_url.postgres_configured()
...
def _connect() -> Iterator[Any]:
    conn = psycopg2.connect(db_url.postgres_url(), connect_timeout=12)
    try:
        yield conn
        conn.commit()
    except Exception:
...
def record_vss_login_attempt(*, reason: str = "10023") -> None:
    now = datetime.now(timezone.utc)
    set_meta(
        LAST_VSS_LOGIN_KEY,
        {"at": now.isoformat(), "reason": reason or "10023"},
    )


def seconds_since_vss_login_attempt() -> float | None:
    meta = get_meta(LAST_VSS_LOGIN_KEY)
    if not meta:
        return None
    at_raw = meta.get("at")
    if not at_raw:
    ...
    try:
        ...
        return None
    return max(0.0, (datetime.now(timezone.utc) - at_dt).total_seconds())
